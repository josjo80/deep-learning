...
class AblationEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.P_mag = 5. #magnitude of power deposited to tissue in W

        self.D = 0. #Damage factor
    ...
    def _step(self, action):
        ...
        #Assumes dt timestep.
        self.D += (self.A*math.exp(-self.dE/(self.R*(T + 273.15)))*self.dt)
        ...
        T = (1/(self.mass*self.C))*((P*self.dt) + self.mass*self.C*T) + 1*self.D #Added in dependancy on D to mimic increase in T as tissue is cooked.
        if T > 100:
            T = 100 #T in Celcius.
        ...
        Z = 1/(sig*self.height) + (10*self.D)**2 #Adding dependance on D damage, to mimic the increase in Z as tissue is cooked.
        ...
        return np.array(self.state), self.D, reward, done, {}

    def _reset(self):
        self.D = 0. #Initialize damage to 0
        self.state = np.array([5., 37., 65., 0])  #[5., self.np_random.uniform(low=35., high=39., size=(1,)), 0., self.np_random.uniform(low=55, high=75, size=(1,)), 0]
        ...
        self.mass = self.length*self.width*self.height*self.density
        self.steps_beyond_done = None
        return np.array(self.state), self.D, self.length, self.width, self.height, self.mass, self.steps_beyond_done
